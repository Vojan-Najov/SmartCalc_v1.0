...
static const char *entry_lbl[] = {"df_min", "df_max", "ef_min", "ef_max"};
...
static void drawing_adaptive_grid(cairo_t *cr_src, int width, int height,
                                  double dmin, double dmax, double emin,
                                  double emax);

static void drawing_plot(cairo_t *cr, int width, int height, double dmin,
...
static void drawing_scale_x(cairo_t *cr, int width, int height, double dmin,
                            double dmax, double emin, double emax);

static void drawing_scale_y(cairo_t *cr, int width, int height, double dmin,
                            double dmax, double emin, double emax);
...
static void init_affine_transform(sc_affine_transform_t *transform, double x0,
                                  double x1, double x2, double y0, double y1,
                                  double y2);
...
static void drawing_adaptive_grid(cairo_t *cr_src, int width, int height,
  ...
  cairo_rectangle(cr_src, width / 2 + (dmin_tmp - (dmin + dif / 2)) * wscale,
...
static void drawing_plot(cairo_t *cr_src, int width, int height, double dmin,
                         double dmax, double emin, double emax) {
  double dif = dmax - dmin;
  ...
  if (sc_function_status() == SC_FUNC_SET) {
    ...
    surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, height);
    cr = cairo_create(surface);
    init_affine_transform(&transform, -(dmin + dif / 2.0), wscale,
                          (double)width / 2.0, -(emin + dif / 2.0), -hscale,
                          (double)height / 2.0);
...
static void drawing_scale_x(cairo_t *cr, int width, int height, double dmin,
  ...
  delta =
      (dmax - dmin > emax - emin) ? (dmax - dmin) / 20.0 : (emax - emin) / 20.0;

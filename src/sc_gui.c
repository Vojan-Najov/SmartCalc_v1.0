...
static const char *get_str_from_label(const char *label, const char *bufstr,
                                      guint buflen);
...
static void app_activate_cb(GApplication *app) {
  ...
  g_signal_connect(entry, "activate", G_CALLBACK(calc_entry_activate_cb),
                   build);
  entry = GTK_WIDGET(gtk_builder_get_object(build, "df_min"));
  g_signal_connect(entry, "activate", G_CALLBACK(plot_entry_activate_cb),
                   build);
  ...
  gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(area), draw_function, build,
                                 NULL);
...
static void handle_calc_output(const char *dst, const char *entry_buf_str,
  ...
  gtk_text_buffer_get_end_iter(tv_buf, &tv_buf_iter);
  gtk_text_buffer_insert(tv_buf, &tv_buf_iter, entry_buf_str, -1);
  gtk_text_buffer_get_end_iter(tv_buf, &tv_buf_iter);
